    ...
    if (!muterole) {
        if (!message.guild.me.permissions.has("MANAGE_ROLES"))
            return message.channel.send(
                "I don't have permission to create the mute role :("
            );
        if (!message.guild.me.permissions.has("MANAGE_CHANNELS"))
            return message.channel.send("I can't apply the role to the channels");
        try {
            message.channel.send(
                'This is the first time that command was used, keep in mind that for the command to work add the "muted" role to new channels'
            );
