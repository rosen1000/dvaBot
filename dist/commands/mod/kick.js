...
module.exports = {
    ...
    run: (bot, message, args) => __awaiter(this, void 0, void 0, function* () {
        ...
        if (!message.guild.me.hasPermission("KICK_MEMBERS"))
            return message.channel.send("Psst... Tell the admins that i can't kick...");
        if (!args[0])
            return message.channel.send("Who should i kick again?");
        let member = getMember(message, args[0]);
        if (!member)
            message.channel.send("Who should i kick again?");
        if (message.author.id == member.id)
            return message.channel.send("There's a leave button 'ya know?");
        if (member.kickable)
            return message.channel.send("I can't kick him tho.. He is more powerfull than me");
